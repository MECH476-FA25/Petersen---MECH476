...
blanks <- read.csv("../data/AIRLIFT_mass_repeatability.csv") %>% transmute(id = as.factor(Filter.ID), date = dmy(Date), mass_mg = Mass..g.*1000)                                                                       
...
blanks %>% group_by(id) %>% summarise(mean = round(mean(mass_mg),3), median = median(mass_mg), sd = sd(mass_mg))
