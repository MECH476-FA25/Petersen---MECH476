...
PA_data_merged <- map_dfr(list.files("../data/purpleair", 
                                     pattern = "\\.csv$", 
                                     full.names = TRUE), 
                          import.w.name) %>% 
  mutate(datetime.Pct = lubridate::ymd_hms(datetime)) %>% 
  rename(current_T_f = current_temp_f, 
         current_hmdty = current_humidity)
...
# XL: you've removed NA in Question 2 when you modify the function `import.w.name().`
...
# XL: Good job! Please take a few minutes to think about the questions in Question 5. Do the data have a central tendency? Do they appear normally distributed? Do events show up in the time series?
